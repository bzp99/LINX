...
from gym_linx.envs.linx_env_cont import LINXEnvCont
import gym_linx.lib.helpers as ATENAUtils
from gym_linx.lib.helpers import (
    ActionVectorEntry,
)
...
import gym_linx.global_env_prop as gep
...
class AgentAnalyzer(object):
    ...
    def __init__(self, model_dir_path=model_dir_path, command_args=command_args, n_sessions_per_dataset=250, seed=0,
        ...
        if is_greedy:
            ...
            self.env = LINXEnvCont(max_steps=command_args["max_steps"])
            ...
            command = "train.py --env LINXcont-v0 --seed %d --demo %s --load %s" % (seed, command_args, model_dir_path)
            sys.argv = command.split()
            self.agent, self.env, self.args = initialize_agent_and_env()
            # get the "real" LINXEnvCont instance
        ...
        for dataset_data in data:
            for info_hist in dataset_data:
                for step, step_info in enumerate(info_hist):
                    ...
                    disc_action_vect = LINXEnvCont.cont2dis(disc_action_vect)
    ...
    def get_vector_of_df_in_each_step(info_hist):
        ...
        result_vector = []
        for step_info in info_hist:
            df_dt, is_grouping = LINXEnvCont.get_filtered_only_or_grouped_data_frame(step_info.info["raw_display"])
