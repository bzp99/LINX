...
# from gym_linx.envs.session_graph import create_tree_from_env
from dep_tregex.dep_tregex import parse_pattern, TregexTree
from gym_linx.lib.tokenization import tokenize_column, get_nearest_neighbor_token, gep
#from gym_linx.reactida.utils.utilities import Repository
from gym_linx.reactida.utils.distance import display_distance
...
class LINXEnvCont(gym.Env):
    ...
    def __init__(self, max_steps=cfg.MAX_NUM_OF_STEPS, ret_df=False, gradual_training=False):
        ...
        LINXEnvCont.len_single_display_vec = len(z) + len(z2) + len(z3)
    ...
    def static_param_softmax_idx_to_action(idx):
        ...
        """
        result, _, _ = LINXEnvCont._param_softmax_idx_to_action_helper(idx)
        return result
    ...
    def create_tree_from_env(self, lightweight=False):  # type: (LINXEnvCont) -> RenderTree
        # dhist = env.dhist
        ahist = self.ahist
